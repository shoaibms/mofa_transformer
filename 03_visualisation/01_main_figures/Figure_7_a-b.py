# -*- coding: utf-8 -*-
"""
Generates the Top 50 Features SHAP Importance Clustermap (Heatmap)
for Leaf and Root pairings based on pre-calculated SHAP importance data.

Reads SHAP importance CSV files generated by the main SHAP analysis script
(e.g., analyze_transformer_shap_ggl_5.py) and saves the clustermap plots
to a specific output directory for figure generation.

Reads data from: SHAP_DATA_DIR
Saves plots to: OUTPUT_PLOT_DIR
"""
...
# --- Paths ---
# Base directory structure assumed from the request
BASE_DIR = r"C:/Users/ms/Desktop/hyper"

# *** INPUT ***: Directory where the original SHAP script saved the CSV data
# This path should match SHAP_DATA_DIR from analyze_transformer_shap_ggl_5.py
SHAP_DATA_DIR = os.path.join(BASE_DIR, "output", "transformer", "shap_analysis_ggl", "importance_data")

# *** OUTPUT ***: Specific directory for saving the Figure 8 heatmap plots
OUTPUT_PLOT_DIR = os.path.join(BASE_DIR, "output", "transformer", "novility_plot")
LOG_DIR = os.path.join(OUTPUT_PLOT_DIR, "logs") # Logs for this specific script
...
# These should generally match the settings used in the original SHAP script
# for consistency, especially those affecting the clustermap.
TARGET_COLS = ['Genotype', 'Treatment', 'Time Point'] # Changed 'Day' to 'Time Point'
PAIRINGS_TO_PROCESS = ["Leaf", "Root"]

# Clustermap specific parameters (copied from analyze_transformer_shap_ggl_5.py)
FIG_DPI = 300
TOP_N_FEATURES_HEATMAP = 50 # For clustermap (as requested)
FIG_SIZE_SQUARE = (7, 7)
FONT_SIZE_TITLE = 14
FONT_SIZE_LABEL = 14
FONT_SIZE_TICK = 14
FONT_SIZE_LEGEND = 14
FONT_SIZE_LEGEND_TITLE = 15
# Use colors from colour.py, providing fallbacks if keys are missing
FEATURE_TYPE_PALETTE = {
    "Spectral": COLORS.get('Spectral', 'skyblue'), # Use colour.py or default skyblue
    "Molecular": COLORS.get('Metabolite', 'lightcoral'), # Use 'Molecular' as the label
    "Unknown": COLORS.get('UnknownFeature', 'grey') # Use colour.py UnknownFeature or default grey
}
CLUSTERMAP_CMAP = 'BuGn' # Changed from 'YlGnBu' to a lighter colormap
...
def load_combined_shap_data(pairing: str, shap_data_dir: str, target_cols: list) -> pd.DataFrame | None:
    ...
    for task_name in target_cols:
        # Handle the special case for 'Time point' in filenames (which might still be 'Day' in file names)
        csv_task_name = 'Day' if task_name.lower() == 'time point' else task_name
        ...
        if os.path.exists(csv_filepath):
            ...
            try:
                ...
                # Basic validation
                if not all(col in task_df.columns for col in ['Feature', 'MeanAbsoluteShap', 'FeatureType', 'Task', 'Pairing']):
                ...
                if task_df['Task'].iloc[0] != task_name or task_df['Pairing'].iloc[0] != pairing:
                    logger.warning(f"    File {csv_filename} has mismatched Task/Pairing metadata. Skipping.")
                    continue

                # Update 'Metabolite' feature type to 'Molecular'
                task_df.loc[task_df['FeatureType'] == 'Metabolite', 'FeatureType'] = 'Molecular'
                # Also handle the old value in case it's still present in some files
                task_df.loc[task_df['FeatureType'] == 'Molecular feature', 'FeatureType'] = 'Molecular'
    ...
    if not found_any:
        logger.error(f"No valid SHAP importance CSV files were found for pairing '{pairing}' in {shap_data_dir}. Cannot generate heatmap.")
        return None

    if all_task_dfs:
        combined_shap_df = pd.concat(all_task_dfs, ignore_index=True)
        logger.info(f"Successfully loaded and combined data for {len(all_task_dfs)} tasks for pairing '{pairing}'. Total rows: {len(combined_shap_df)}")
        ...
        if 'FeatureType' not in combined_shap_df.columns:
            ...
            # This part is less ideal, we should ideally know the feature type.
            # Assuming Feature name might contain hints, but safer to mark as Unknown.
            combined_shap_df['FeatureType'] = 'Unknown'
        else:
            combined_shap_df['FeatureType'] = combined_shap_df['FeatureType'].fillna('Unknown')

        # Transform feature names: replace N_Cluster_XXXX with N_XXXX and P_Cluster_XXXX with P_XXXX
        combined_shap_df['Feature'] = combined_shap_df['Feature'].apply(
            lambda x: x.replace('N_Cluster_', 'N_').replace('P_Cluster_', 'P_') if isinstance(x, str) else x
        )

        return combined_shap_df
    else:
        # This case should be covered by found_any check, but added for safety.
        logger.error(f"Failed to load any SHAP data for pairing '{pairing}'.")
        return None


# ===== PLOTTING FUNCTION (Adapted from original script) =====
def plot_shap_clustermap(shap_data: pd.DataFrame, pairing: str, config: dict):
    ...
    Saves the plot to the specified output directory in the config.
    """
    logger.info(f"--- Generating SHAP Clustermap for {pairing} ---")

    # *** Use the specific OUTPUT directory for this script ***
    ...
    palette = config['FEATURE_TYPE_PALETTE'] # Use the palette from config
    ...
    try:
        ...
        if not all(col in shap_data.columns for col in required_cols):
            ...
            logger.error(f"Combined SHAP data missing required columns: {missing}. Cannot create clustermap.")
            return

        # Ensure 'Unknown' is in palette if present in data
        if 'Unknown' in shap_data['FeatureType'].unique() and 'Unknown' not in palette:
            palette['Unknown'] = 'grey' # Add default grey for Unknown if missing

        # Find the top N features based on the maximum MeanAbsoluteShap across all tasks
        # Group by feature, find the row index with the max SHAP value for that feature
        idx_max = shap_data.loc[shap_data.groupby('Feature')['MeanAbsoluteShap'].idxmax()]
        # Get the top N features from these maximums
        top_features = idx_max.nlargest(top_n, 'MeanAbsoluteShap')['Feature'].tolist()

        if not top_features:
            logger.warning(f"Could not determine top {top_n} features for {pairing}. The SHAP data might be empty or have zero importance values.")
            return
        ...
        plot_data = shap_data[shap_data['Feature'].isin(top_features)].copy() # Use .copy() to avoid SettingWithCopyWarning
        ...
        # Need to handle cases where a feature might appear multiple times if aggregation wasn't perfect
        # or if FeatureType varies (shouldn't happen with correct input data).
        feature_types = plot_data[['Feature', 'FeatureType']].drop_duplicates().set_index('Feature')
        # Align feature types with the pivot table index
        feature_types = feature_types.reindex(pivot_table.index)
        # Map types to colors, handling potential missing types or features
        row_colors_series = feature_types['FeatureType'].map(palette).fillna('grey') # Use grey for any unexpected missing types
        row_colors_series.name = 'Feature Type' # Set name for legend title

        # Create the clustermap
        # Note: z_score=0 normalizes rows (features) before clustering/plotting colors
        g = sns.clustermap(pivot_table,
                           cmap=cmap,
                           figsize=figsize,
                           row_colors=row_colors_series if not row_colors_series.empty else None,
                           linewidths=0.5,
                           linecolor='lightgray',
                           dendrogram_ratio=(.2, .1), # Adjust space for dendrograms
                           cbar_pos=(0.02, 0.8, 0.03, 0.18), # Position color bar
                           z_score=0, # Normalize features (rows) for color visualization
                           annot=False) # Don't annotate cells with numbers

        # --- Customize Appearance ---
        if pairing == "Leaf":
            title_text = "a      Tissue-Specific Predictive Patterns in Leaf"
        elif pairing == "Root":
            title_text = "b      Tissue-Specific Predictive Patterns in Root"
        else:
            title_text = f'Top {len(top_features)} Feature SHAP Importance ({pairing})'
        g.ax_heatmap.set_title(title_text, fontsize=config['FONT_SIZE_TITLE'], pad=30, weight='bold')
        ...
        g.ax_heatmap.tick_params(axis='x', labelsize=config['FONT_SIZE_TICK'], rotation=0) # Keep task names horizontal
        # Reduce y-tick label size slightly for potentially many features
        g.ax_heatmap.tick_params(axis='y', labelsize=max(6, config['FONT_SIZE_TICK'] - 2))
        # Ensure y-axis labels are not cut off
        plt.setp(g.ax_heatmap.get_yticklabels(), rotation=0)

        # --- Add Legend for Row Colors (Omics Type) ---
        try:
            # Create handles only for the types present in the plotted data
            present_types = feature_types['FeatureType'].unique()
            handles = [plt.Rectangle((0, 0), 1, 1, color=palette[ftype]) for ftype in present_types if ftype in palette]
            labels = [ftype for ftype in present_types if ftype in palette]

            if handles: # Only add legend if there are valid types with colors
                # Place legend inside the heatmap axis on the top-right
                g.ax_heatmap.legend(handles=handles, labels=labels, title='Feature Type',
                             loc='upper right', # Position top-right inside the plot
                             frameon=True,
                             framealpha=0.7, # Set background with opacity
                             fontsize=config.get('FONT_SIZE_LEGEND', 10),
                             title_fontsize=config.get('FONT_SIZE_LEGEND_TITLE', 12))
...
def main():
    ...
    plot_config = {
        ...
        'FONT_SIZE_LEGEND_TITLE': FONT_SIZE_LEGEND_TITLE,
        'FEATURE_TYPE_PALETTE': FEATURE_TYPE_PALETTE,
        'FIG_DPI': FIG_DPI,
    }
    ...
    for pairing in PAIRINGS_TO_PROCESS:
        ...
        logger.info(f"===== Finished processing for {pairing}. Duration: {pairing_end_time - pairing_start_time} =====")
...
if __name__ == '__main__':
    try:
        main()
         ...
         logger.critical(f"A critical error occurred during the main execution workflow: {e}", exc_info=True)
         sys.exit(1)