...
TEXT = {
    ...
    'main_title': 24,         # Figure Title (Increased)
    'panel_label': 21,        # Panel Labels (Increased)
    'panel_title': 19,        # Title for each subplot/panel (Increased)
    'axis_label': 19,         # X and Y axis labels (Increased)
    'tick_label': 18,         # Axis tick numbers/text (Increased)
    'legend_title': 21,       # Title of the legend box (Increased)
    'legend_text': 18,        # Text for individual legend items (Increased)
    'annotation': 17,         # Text annotations within the plot area (Increased)
    'caption': 17,            # Figure caption text (Increased)
    'table_header': 17,       # Text in table headers (Increased)
    'table_cell': 17,         # Text in table cells (Increased)
    # Added back tissue_label from original script, with increased size
    'tissue_label': 16        # Tissue labels (Leaf, Root) (Increased)
}

# Network visualization parameters
NETWORK = {
    'node_size_min': 100,        # Minimum node size
    'node_size_max': 800,        # Maximum node size
    'edge_width_min': 0.8,       # Minimum edge width
    'edge_width_max': 6,         # Maximum edge width
    'edge_alpha': 0.7,           # Edge transparency
    'node_alpha': 0.9,           # Node transparency
    'node_edge_width': 0.5,      # Node border width
    'label_offset_x': 5,         # Node label x offset
    'label_offset_y': 0,         # Node label y offset
    'n_top_connections': 40,     # Number of top connections to display
    'min_attention': 0.01        # Minimum attention score to include
}

# Bar chart parameters
BARS = {
    'bar_width': 0.35,           # Width of bars
    'bar_alpha': 0.9,            # Bar transparency
    'value_offset': 3,           # Offset for value annotations
    'percent_offset': 1.1,       # Multiplier for percent difference height
    'ylim_buffer': 1.15,         # Additional space above bars (multiplier)
    'edge_color': 'black',       # Bar edge color
    'edge_width': 0.5            # Bar edge width
}

# Layout parameters
LAYOUT = {
    'figure_width': 20,          # Figure width in inches
    'figure_height': 20,         # Figure height in inches
    'top_margin': 0.90,          # Top margin for layout (adjust for legends)
    'bottom_margin': 0.05,       # Bottom margin for layout
    'left_margin': 0,            # Left margin for layout
    'right_margin': 1,           # Right margin for layout
    'colorbar_width': 0.4,       # Width of the colorbar
    'colorbar_height': 0.02,     # Height of the colorbar
    'colorbar_position_x': 0.3,  # X position of colorbar left edge
    'colorbar_position_y': 0.32  # Y position of colorbar bottom edge
}

# Define a custom colormap for edge weights
edge_color_map = LinearSegmentedColormap.from_list('attention_cmap', [
    (0.0, COLORS['Edge_Low']),    # Very light gray for lowest values
    (0.2, '#d1e5f0'),            # Light blue
    (0.4, '#92c5de'),            # Medium blue
    (0.6, '#4393c3'),            # Darker blue
    (0.8, '#2166ac'),            # Deep blue
    (1.0, COLORS['Edge_High'])    # Very dark blue for highest values
])
...
def load_data_safe(filepath, default_name="Unknown"):
    ...
    try:
        ...
        print(f"Successfully loaded {default_name} data: {data.shape} rows, {len(data.columns)} columns")
        return data
    except Exception as e:
        print(f"Error loading data from {filepath}: {e}")
        # Return empty DataFrame with expected columns
        return pd.DataFrame(columns=['Genotype', 'Treatment', 'Time point', 'Spectral_Feature',
                                    'Molecular_Feature', 'Mean_Attention_S2M_Group_AvgHeads'])
...
def clean_metadata_columns(df):
    ...
    if 'Genotype' in df.columns:
        df['Genotype'] = df['Genotype'].astype(str).replace({'1': 'G1', '1.0': 'G1',
                                                            '2': 'G2', '2.0': 'G2'})
    ...
    if 'Treatment' in df.columns:
        df['Treatment'] = df['Treatment'].astype(str).replace({'0': 'T0', '0.0': 'T0',
                                                               '1': 'T1', '1.0': 'T1'})
    ...
    if 'Time point' in df.columns:
        ...
        df = df.dropna(subset=['Time point'])  # Remove rows where Time point conversion failed
...
def prepare_network_data(df, tissue, n_top_connections=40, min_attention=0.01):
    ...
    if len(day3_df) < 10:
        ...
        if len(day3_df) < 10:
            print(f"WARNING: Still few data points for peak time point. Using all time points under stress.")
    ...
    for spec, molecular_feature in common_pairs:
        g1_row = g1_top[(g1_top['Spectral_Feature'] == spec) & (g1_top['Molecular_Feature'] == molecular_feature)]
        g2_row = g2_top[(g2_top['Spectral_Feature'] == spec) & (g2_top['Molecular_Feature'] == molecular_feature)]
...
def categorize_molecular_feature(feature):
    ...
    if feature_str.startswith('P_'):
        return "P", COLORS['P'] # Renamed from P_Cluster

    # Negative mode features (often associated with organic acids, phenolics)
    elif feature_str.startswith('N_'):
        return "N", COLORS['N'] # Renamed from N_Cluster

    # Default category
    else:
        return "Other Molecular", COLORS['Molecular_feature_Other'] # Renamed from Other

def calculate_node_importance(data):
    """Calculate node importance metrics for sizing"""
    # Initialize dictionaries
    spectral_importance = {}
    molecular_feature_importance = {} # Renamed from metabolite_importance
    ...
    for _, row in combined_df.iterrows():
        ...
        molecular_feature = row['Molecular_Feature'] # Renamed from metab
    ...
    G = nx.Graph()
    for _, row in combined_df.iterrows():
        G.add_edge(row['Spectral_Feature'], row['Molecular_Feature'], # Renamed from Metabolite_Feature
    ...
    for node, centrality in degree_centrality.items():
        ...
        if node in spectral_importance:
            ...
            norm_attention = spectral_importance[node] / max(spectral_importance.values()) if spectral_importance else 0
            # Combine metrics
        ...
        if node in molecular_feature_importance:
            norm_attention = molecular_feature_importance[node] / max(molecular_feature_importance.values()) if molecular_feature_importance else 0
            molecular_feature_importance[node] = 0.5 * norm_attention + 0.5 * centrality
    
    return spectral_importance, molecular_feature_importance # Renamed second return value
...
def create_network_visualization(fig, ax, data, geno, tissue, node_importances, pos=None):
    ...
    spec_importance, molecular_feature_importance = node_importances # Renamed second var
    ...
    for spec in data['all_spectral']:
        ...
        G.add_node(spec, bipartite=0, type='spectral', category=category, color=color, importance=importance)
    
    for molecular_feature in data['all_molecular_feature']: # Renamed var and dict key
        category, color = categorize_molecular_feature(molecular_feature) # Updated function call
        # Get importance or set default
        importance = molecular_feature_importance.get(molecular_feature, 0.001) # Renamed dict and var
        G.add_node(molecular_feature, bipartite=1, type='molecular_feature', category=category, color=color, importance=importance) # Renamed type
    ...
    for _, row in df.iterrows():
        spec = row['Spectral_Feature']
        molecular_feature = row['Molecular_Feature'] # Renamed var
        attention = row['Mean_Attention_S2M_Group_AvgHeads']
        
        # Only add edge if both nodes exist
        if spec in G.nodes and molecular_feature in G.nodes: # Renamed var
            G.add_edge(spec, molecular_feature, weight=attention) # Renamed var
            edge_weights.append(attention)
            edge_pairs.append((spec, molecular_feature)) # Renamed var
    ...
    if pos is None:
        ...
        molecular_feature_nodes = {n for n, d in G.nodes(data=True) if d['type'] == 'molecular_feature'} # Renamed var and type
        ...
        molecular_feature_matrix = np.zeros((len(molecular_feature_nodes), len(molecular_feature_nodes))) # Renamed var
        molecular_feature_list = list(molecular_feature_nodes) # Renamed var
        ...
        for i, molecular_feature1 in enumerate(molecular_feature_list): # Renamed var
            for j, molecular_feature2 in enumerate(molecular_feature_list): # Renamed var
                if i == j:
                    continue
                # Count number of shared spectral connections
                molecular_feature1_connections = set(G.neighbors(molecular_feature1)) # Renamed var
                molecular_feature2_connections = set(G.neighbors(molecular_feature2)) # Renamed var
                shared = len(molecular_feature1_connections.intersection(molecular_feature2_connections))
                # More shared connections = smaller distance
                molecular_feature_matrix[i, j] = 1.0 / (shared + 1) # Renamed var
        ...
        try:
            ...
            molecular_feature_order = list(range(len(molecular_feature_list))) # Renamed var
        ...
        for i, idx in enumerate(molecular_feature_order): # Renamed var
            if idx < len(molecular_feature_list):  # Safety check, Renamed var
                node = molecular_feature_list[idx] # Renamed var
                # Position based on clustering with slight vertical jitter
                jitter = np.random.uniform(-0.02, 0.02)
                pos[node] = (0.7, 0.8 - (i / max(1, len(molecular_feature_order)) * 1.6) + jitter) # Renamed var
    ...
    for node in G.nodes():
        ...
        elif G.nodes[node]['type'] == 'molecular_feature': # Check for new type
            importance = G.nodes[node]['importance']
            size = importance * 5000
        ...
        node_sizes.append(max(NETWORK['node_size_min'], min(NETWORK['node_size_max'], size)))
    ...
    if edge_weights:
        ...
        edge_widths = [max(NETWORK['edge_width_min'], min(NETWORK['edge_width_max'], w * 25)) for w in edge_weights]
    else:
        edge_colors = ['#d3d3d3']
        edge_widths = [0.5]
    ...
    if lines:
        lc = LineCollection(lines, colors=colors, linewidths=linewidths, alpha=NETWORK['edge_alpha'], zorder=1)
    ...
    for i, node in enumerate(G.nodes()):
        if node in pos:  # Safety check
            ax.scatter(pos[node][0], pos[node][1], s=node_sizes[i], 
                      c=node_colors[i], edgecolor='black', linewidth=NETWORK['node_edge_width'], 
                      alpha=NETWORK['node_alpha'], zorder=2)
    
    # Add node labels for top nodes (top 3 spectral and top 3 molecular feature by importance)
    spec_nodes = {n: G.nodes[n]['importance'] for n, d in G.nodes(data=True) if d['type'] == 'spectral'}
    molecular_feature_nodes = {n: G.nodes[n]['importance'] for n, d in G.nodes(data=True) if d['type'] == 'molecular_feature'} # Renamed var and type
    
    # Sort by importance
    top_spec = sorted(spec_nodes.items(), key=lambda x: x[1], reverse=True)[:3]
    top_molecular_feature = sorted(molecular_feature_nodes.items(), key=lambda x: x[1], reverse=True)[:3] # Renamed var

    for node, _ in top_spec + top_molecular_feature: # Renamed var
        ...
        # Shorten long labels
        if len(label) > 12:
            if 'Cluster' in label or label.startswith('P_') or label.startswith('N_'): # Adjusted check
...
def create_network_stats_visualization(fig, network_metrics_df, ax_row_idx):
    ...
    metric_display_names = {
        'Avg_Attention_Top100': 'Coordination Strength',
        'StdDev_Attention': 'Network Focus',
        'P95_Attention': 'Peak Connection Strength'
    }
    ...
    if metric_data_list:
        ...
        ax.set_xticklabels(group_labels, rotation=45, ha='right', fontsize=TEXT['tick_label'])
        ...
        ax.set_title("Genotype-specific coordination network properties under peak stress",
...
# Define a custom colormap for edge weights - used in both parts
edge_color_map = LinearSegmentedColormap.from_list('attention_cmap', [
    (0.0, '#f0f0f0'),  # Very light gray for lowest values
    (0.2, '#d1e5f0'),  # Light blue
    (0.4, '#92c5de'),  # Medium blue
    (0.6, '#4393c3'),  # Darker blue
    (0.8, '#2166ac'),  # Deep blue
    (1.0, '#053061')   # Very dark blue for highest values
])
...
if leaf_data:
    leaf_spec_importance, leaf_molecular_feature_importance = calculate_node_importance(leaf_data) # Renamed var
else:
    leaf_spec_importance, leaf_molecular_feature_importance = {}, {} # Renamed var

if root_data:
    root_spec_importance, root_molecular_feature_importance = calculate_node_importance(root_data) # Renamed var
else:
    root_spec_importance, root_molecular_feature_importance = {}, {} # Renamed var

# Create the integrated figure
print("\nCreating integrated visualization...")
fig = plt.figure(figsize=(20, 20))  # Reduced height to make barplots shorter

# Create network visualizations in top two rows (2x2 grid)
# We'll use subplot2grid to create a 3x4 grid
...
if leaf_data:
    ...
    ax_leaf_g1.text(0.5, 0.5, "Leaf - G1: Data not available", ha='center', va='center', fontsize=14)
...
if leaf_data:
    ...
    ax_leaf_g2.text(0.5, 0.5, "Leaf - G2: Data not available", ha='center', va='center', fontsize=14)
...
if root_data:
    ...
    ax_root_g1.text(0.5, 0.5, "Root - G1: Data not available", ha='center', va='center', fontsize=14)
...
if root_data:
    ...
    ax_root_g2.text(0.5, 0.5, "Root - G2: Data not available", ha='center', va='center', fontsize=14)

# Add panel labels for all subplot components with proper formatting
# Leaf networks
...
# Add F) label for colorbar - repositioned to match the reference image
# fig.text(0.28, 0.97, 'F)', fontsize=22, fontweight='bold')
...
if stats_axes: # Add panel label for the stats plot
    stats_axes[0].text(-0.06, 1.1, 'e', fontsize=22, fontweight='bold',
                       ha='center', va='center', transform=stats_axes[0].transAxes)

# Create common legends for the network visualizations
# Make space for network visualization legend below the networks
fig.subplots_adjust(bottom=0.2)

# Create legend for node categories - adjusted colors to exactly match visualization
spectral_elements = [
...
molecular_feature_elements = [ # Renamed var
    mpatches.Patch(color=COLORS['P'], label='P Molecular Features'), # Updated key and label
    mpatches.Patch(color=COLORS['N'], label='N Molecular Features')  # Updated key and label
]

# Create size legend
size_elements = [
    Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=8, label='Low Importance'),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=12, label='Medium Importance'),
    Line2D([0], [0], marker='o', color='w', markerfacecolor='gray', markersize=16, label='High Importance')
]

# Create legends with all elements moved to the right-hand side - SIGNIFICANTLY INCREASED font size
...
# Move the colorbar to top with larger text size - NO PANEL LABEL
cbar_ax = fig.add_axes([0.3, 0.97, 0.4, 0.015])  # Positioned at top of figure
...
# NO panel label for the colorbar - it's just a legend

# Remove the main suptitle - as requested

# Add vertical spacing between subplot rows and adjust figure layout
fig.subplots_adjust(hspace=3.5)  # Further increased vertical space

# Adjust layout to give bar plot full width but with more space at bottom
plt.tight_layout(rect=[0, 0.15, 0.85, 0.92])  # Increased bottom margin to 0.15 (from 0.05)

# Manually adjust the bottom bar plot to use full width but be higher up
for i in range(len(fig.axes)):
    if i == 4:  # The bar plot is the 5th axes (index 4)
        # Lowered bottom from 0.15 to 0.12 to add space above
        fig.axes[i].set_position([0.1, 0.12, 0.8, 0.25])
        # If it overlaps the caption, try reducing height too:
        # fig.axes[i].set_position([0.1, 0.12, 0.8, 0.23])

# Remove the plot description/caption
# Caption has been removed as requested

output_path = os.path.join(output_dir, "fig3_integrated_networks_and_statistics.png")
plt.savefig(output_path, dpi=300, bbox_inches='tight')
plt.close()

print(f"\nIntegrated Figure 3 saved to: {output_path}")
