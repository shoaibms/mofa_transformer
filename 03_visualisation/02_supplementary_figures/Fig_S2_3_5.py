...
plt.rcParams.update({
    ...
    'font.sans-serif': FONTS_SANS['sans_serif'], # Changed underscore to hyphen
    'font.size': FONTS_SANS['tick_label'], # Default font size
...
try:
    ...
    leaf_stats = pd.DataFrame({
        ...
        'Metabolite_Feature': ['N_1909'] * 30,  # Renamed N_Cluster_ -> N_
        'Time point': [1, 2, 3] * 10,          # Renamed Day -> Time point
    ...
    root_stats = pd.DataFrame({
        ...
        'Metabolite_Feature': ['N_1234'] * 30,  # Renamed N_Cluster_ -> N_
        'Time point': [1, 2, 3] * 10,          # Renamed Day -> Time point
        'G1_Mean': np.random.uniform(0.02, 0.05, 30),
        'G2_Mean': np.random.uniform(0.01, 0.03, 30),
        'Difference': np.random.uniform(0.01, 0.03, 30),
    ...
    top_pairs = pd.DataFrame({
        ...
        'Metabolite_Feature': ['N_1909'] * 25 + ['N_1234'] * 25, # Renamed N_Cluster_ -> N_
...
leaf_color = COLORS['Leaf']  # Blue -> Green
root_color = COLORS['Root']  # Pink -> Teal
...
fold_change_cmap = LinearSegmentedColormap.from_list(
    "fold_change", [COLORS['Background'], COLORS['G1']] # Use G1 color as target
)
...
def create_tissue_comparison_plot():
    ...
    # First, get the top pairs in each tissue
    ...
    # Create figure
    fig = plt.figure(figsize=(20, 16)) # Increased width from 18 to 20
    # Increase hspace significantly to create more vertical space between rows 2 and 3
    # Increased wspace from 0.3 to 0.4 to make heatmap columns slightly narrower
    ...
    leaf_summary_hm, leaf_heatmap = create_significance_heatmap(ax1, leaf_stats, leaf_top, 'Leaf', vmin=vmin, vmax=vmax, cbar=False) # Pass args

    # Panel B: Top Pairs and Their Significance Across Time points - Root
    ax2 = fig.add_subplot(gs[0, 1])
    root_summary_hm, root_heatmap = create_significance_heatmap(ax2, root_stats, root_top, 'Root', vmin=vmin, vmax=vmax, cbar=False) # Pass args
    ...
    # Keep the sorted order from create_diff_fold_scatter logic (Time point, then Significance)
    ...
    # Position it to the right of the heatmaps
    cbar_ax = fig.add_axes([0.93, gs[0,1].get_position(fig).y0, 0.015, gs[0,1].get_position(fig).height]) # Pass fig
    cbar = fig.colorbar(leaf_heatmap.collections[0], cax=cbar_ax) # Use mappable from one heatmap
    ...
    fig.legend(sorted_handles, sorted_labels,
               loc='upper center', # Use upper center to align with anchor point below plots
               bbox_to_anchor=(0.5, 0.42), # Adjusted y position up slightly from 0.38
               ncol=3,
    ...
    # Increase bottom margin significantly for caption and legend
    plt.tight_layout(rect=[0, 0.15, 0.92, 0.96]) # Increased bottom from 0.12 to 0.15, kept right reduced for cbar
...
def create_significance_heatmap(ax, stats, top_pairs, tissue, vmin=None, vmax=None, cbar=True): # Add vmin, vmax, cbar
    ...
    # Create the heatmap
    heatmap = sns.heatmap(pivot_df, cmap='BuGn', ax=ax, # Changed cmap to 'BuGn'
                          vmin=vmin, vmax=vmax, # Use passed vmin/vmax
                          cbar=cbar, # Use passed cbar flag
                          cbar_kws={'label': '-log10(p-value)'} if cbar else None) # Conditionally add cbar_kws
    
    # Format the heatmap
    ax.set_title(f"{tissue} Tissue: Statistical Significance Across Time points", fontsize=FONTS_SANS['panel_title']) # Day -> Time point
        ...
        for time_point in pivot_df.columns:
            ...
            if not pair_time_point_data.empty and pair_time_point_data.iloc[0]['Significant']:
                ax.text(time_point - 0.5, pivot_df.index.get_loc(pair) + 0.5, '*',
                       ha='center', va='center', color=COLORS['Text_Dark'], fontsize=FONTS_SANS['annotation'], # Adjusted size
                       fontweight='bold')
    ...
    if cbar and hasattr(ax.collections[0], 'colorbar') and ax.collections[0].colorbar:
        ax.collections[0].colorbar.set_label('-log10(p-value)', fontsize=FONTS_SANS['axis_label']) # Use axis label size for colorbar
    ...
    summary = {
        ...
        'min_neg_log_p': plot_df['Neg_Log_P'].min() # Add min for global scaling
    }
    return summary, heatmap # Return heatmap object
...
def create_diff_fold_scatter(ax, stats, tissue):
    ...
    # --- REMOVED LEGEND PLOTTING ---
    # ax.legend(sorted_handles, cleaned_labels, loc='upper left', fontsize=FONTS_SANS['legend_text'])
...
def create_spectral_distribution(ax, top_pairs, tissue):
    ...
    if not wavelengths:
        if tissue == 'Leaf':
            wavelengths = np.random.normal(650, 50, 30)  # Centered around visible region for leaf
        else:
            wavelengths = np.random.normal(1100, 100, 30)  # Centered around NIR for root
    
    # Define spectral regions
    regions = [
        {'name': 'UV', 'start': 350, 'end': 400, 'color': COLORS['Spectral_UV']},
        {'name': 'Visible (Blue)', 'start': 400, 'end': 500, 'color': COLORS['G1']}, # Using G1 color
        {'name': 'Visible (Green)', 'start': 500, 'end': 600, 'color': COLORS['Leaf']}, # Using Leaf color
        {'name': 'Visible (Red)', 'start': 600, 'end': 700, 'color': COLORS['Negative_Diff']}, # Using Diff color
...
def create_temporal_trajectories(ax, trends, tissue):
    ...
    time_points = [1, 2, 3]
    colors = [COLORS['G1'], COLORS['G2'], COLORS['T0'], COLORS['T1'], COLORS['NonSignificant']]
    ...
    for i, (pattern, key) in enumerate(g1_examples.items()):
        ...
        # Add pattern label
        # ax.text(x_offset + 2, y_max, pattern, # Moved y to y_max
        #        ha='center', va='top', fontsize=FONTS_SANS['annotation'], fontweight='bold', color=COLORS['Text_Annotation'], # Changed va to top
        #        bbox=dict(facecolor=COLORS['Annotation_Box_BG'], alpha=0.8, ec=COLORS['Annotation_Box_Edge']))
        ...
        if len(label_text) > 20:
            ...
            # Keep original if not N_ or P_ format

            label_text = f"{spectral} + {molecular_feature}"
        
        ax.text(x_offset + 2, y_min + y_range * 0.05, label_text,
               ha='center', va='bottom', fontsize=FONTS_SANS['annotation']-2, color=COLORS['Text_Annotation']) # Reduced size
    ...
    ax.set_xticklabels(x_labels, fontsize=FONTS_SANS['tick_label']-2) # Reduced size for x-tick labels
    ...
    ax.tick_params(axis='y', which='major', labelsize=FONTS_SANS['tick_label']) # Apply y tick size (x handled separately)
    ...
    ax.legend(loc='upper right', fontsize=FONTS_SANS['legend_text'], bbox_to_anchor=(1.0, 0.95), framealpha=0.5) # Adjusted legend position slightly & added framealpha
...
def create_biomarker_table(ax, markers, tissue):
    ...
    table = ax.table(
        ...
        loc='center',
        cellLoc='center',
        colColours=[COLORS['Table_Header_BG']] * len(column_labels), # Use Table_Header_BG for all columns
        colWidths=[0.15, 0.18, 0.12, 0.12, 0.15, 0.12, 0.15]
    )
    
    # Style the table
    table.auto_set_font_size(False)
    # table.set_fontsize(FONTS_SANS['table_cell']) # Font size set per cell type below
    table.scale(1, 1.8) # Increased vertical scale from 1.5 to 1.8

    # Style header and cells
    for (row, col), cell in table.get_celld().items():
        if row == 0: # Header row
            cell.set_text_props(fontweight='bold', color=COLORS['Text_Dark'], ha='center', va='center') # Ensure va is center
            cell.set_fontsize(FONTS_SANS['table_header'])
            cell.set_height(0.4) # Explicitly set header row height
        else: # Data rows
            cell.set_fontsize(FONTS_SANS['table_cell'])
            # Center align text in data cells as well
    ...
    for i, row in enumerate(rows):
        ...
        if p_val < 0.001:
            cell.set_facecolor(COLORS['Negative_Diff']) # Strong color for lowest p-values
        elif p_val < 0.01:
            cell.set_facecolor(COLORS['Table_Highlight_BG']) # Highlight color
        elif p_val < 0.05:
