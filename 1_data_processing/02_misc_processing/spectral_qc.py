"""
Hyperspectral Data Quality Assessment Pipeline

This script performs comprehensive quality assessment of hyperspectral data through multiple
analytical steps including outlier detection, normality testing, baseline assessment, and
derivative analysis. It generates detailed reports and visualizations to help identify
potential data quality issues and preprocessing needs.

Key Features:
- Multiple outlier detection methods (IQR, Modified Z-score, LOF)
- Normality assessment using Shapiro-Wilk test
- Baseline trend analysis using Theil-Sen regression
...
######################################################
############ LOGGING SETUP & HELPERS (Unchanged from V4)
######################################################

def setup_logging(log_dir):
    """Initialize logging configuration with file and console handlers."""
    os.makedirs(log_dir, exist_ok=True)
    log_filename = os.path.join(log_dir, f"analysis_log_{datetime.now():%Y%m%d_%H%M%S}.log")
...
def set_plot_styles():
    """Configure matplotlib plot styles and parameters."""
...
def save_report_to_json(report_dict, filename):
    """Save analysis report to JSON file with custom serialization."""
    try:
        def default_converter(o):
            if isinstance(o, (np.int_, np.intc, np.intp, np.int8, np.int16, np.int32,
                ...
                return f"Array shape {o.shape}, Median {np.median(o):.3g}, IQR {iqr(o):.3g}"
...
def extract_wavelengths_from_wide_format(df_columns):
    """Extract wavelength values from column names or use indices if not available."""
    wavelengths = []
    ...
    for col in col_names_str:
        try:
            ...
            logging.warning(f"Could not convert '{col}'. Wavelength axis might use index.")
            wavelengths = list(range(len(df_columns)))
            break
    return np.array(sorted(wavelengths))

def get_iterator(iterable, desc="Processing"):
    """Return tqdm iterator if available, otherwise return regular iterator."""
    return tqdm(iterable, desc=desc, unit="item", leave=False) if TQDM_AVAILABLE else iterable

#############################################################################
# --- STEP 1: LOAD & PREPARE DATA (Unchanged from V4) ---
#############################################################################
def load_and_prepare_data(file_path, spec_start_col):
    """
    Load and prepare hyperspectral data for analysis.
    
    Args:
        file_path (str): Path to the input CSV file
        spec_start_col (int): Column index where spectral data begins
        
    Returns:
        tuple: (metadata, spectral_data, wavelengths, prep_info)
            - metadata: DataFrame containing non-spectral data
            - spectral_data: DataFrame containing spectral measurements
            - wavelengths: Array of wavelength values
            - prep_info: Dictionary with data preparation details
    ...
    try:
        ...
        logging.info(f"Separated: Metadata {metadata.shape}, Spectral {spectral_data_raw.shape}")

        # Handle NaNs in spectral data
        ...
        if initial_nan_count > 0:
            ...
            spectral_data = spectral_data_raw.dropna(
                axis=1,
                thresh=int(0.9 * len(spectral_data_raw))
            )
            dropped_cols = original_cols - spectral_data.shape[1]
            
            if dropped_cols > 0:
                logging.warning(f"Dropped {dropped_cols} spectral columns due to >10% NaNs.")
            
            if spectral_data.isna().sum().sum() > 0:
                logging.warning(
                    f"Filling remaining {spectral_data.isna().sum().sum()} NaNs with column medians."
                )
            ...
            nan_handling_info = (
                f"{initial_nan_count} NaNs initially. Dropped {dropped_cols} cols (>10% NaN). "
                "Remaining filled with median."
            )
        else:
        ...
        logging.error(
            f"FATAL: An unexpected error occurred during data loading/preparation: {e}",
            exc_info=True
        )
        raise
...
# --- Modified: Returns plotting data, does not plot ---
#############################################################################

def perform_core_quality_assessment(spectral_data, wavelengths, output_dir_base):
    """
    Perform core quality assessment of spectral data.
    
    This function performs outlier detection using multiple methods (IQR, Modified Z-score,
    and optionally LOF), calculates basic quality metrics, and prepares data for visualization.
    
    Args:
        spectral_data (pd.DataFrame): Spectral measurements
        wavelengths (np.ndarray): Array of wavelength values
        output_dir_base (str): Base directory for output files
        
    Returns:
        dict: Results containing outlier information, basic metrics, and plotting data
    """
    ...
    if valid_samples < 2:
        ...
        try:
            ...
            iqr_mask = (
                (spectral_data < (Q1 - IQR_MULTIPLIER * IQR_vals)) |
                (spectral_data > (Q3 + IQR_MULTIPLIER * IQR_vals))
            ).any(axis=1)
        ...
        if RUN_LOF_OUTLIERS:
            try:
                ...
                if valid_samples > n_neighbors_lof:
                    ...
                    logging.warning(
                        f"Skipping LOF: Samples ({valid_samples}) <= n_neighbors ({n_neighbors_lof})."
                    )
                    outlier_summary['lof'] = {
                        'count': 'Skipped',
                        'reason': f'Samples {valid_samples} <= n_neighbors {n_neighbors_lof}'
                    }
            except Exception as e:
            ...
            outlier_summary['lof'] = {'count': 'Skipped', 'reason': 'Disabled by config'}
        ...
        if outlier_indices:
            outlier_df = pd.DataFrame({'outlier_sample_index': sorted(list(outlier_indices))})
            outlier_csv_path = os.path.join(output_dir_core, 'core_outlier_indices.csv')
    ...
    if spectral_data.empty:
        ...
        try:
            ...
            if num_bands > 500:
                sample_cols_metric = np.random.choice(num_bands, 500, replace=False)
                corr_matrix_metric = spectral_data.iloc[:, sample_cols_metric].corr()
                logging.info("Sampled 500 bands for mean correlation metric.")
            else:
                corr_matrix_metric = (
                    corr_matrix_sampled if num_bands <= 100 else spectral_data.corr()
                )
...
def perform_normality_assessment(spectral_data, output_dir_base):
    """
    Perform normality assessment of spectral data using Shapiro-Wilk test.
    
    This function tests the normality of each spectral band using the Shapiro-Wilk test,
    calculates summary statistics, and prepares data for visualization.
    ...
    Returns:
        dict: Results containing normality test results, summary statistics, and plotting data
    """
    ...
    detailed_results_filename = os.path.join(
        output_dir_normality,
        'normality_details_shapiro.csv'
    )
    results_df.to_csv(detailed_results_filename, index=False, float_format='%.4g')
    ...
    normal_count = (
        results_df[results_df['is_normal'] == True]['is_normal'].count()
        if tested_bands > 0 else 0
    )
    percent_normal = (
        (normal_count / tested_bands * 100)
        if tested_bands > 0 else 0
    )
    ...
    logging.info(f"Normality summary: {percent_normal:.1f}% of tested bands assessed as normal.")
...
def perform_robust_baseline_assessment(spectral_data, output_dir_base):
    """
    Perform robust baseline assessment of spectral data.
    
    This function analyzes baseline trends using Theil-Sen regression and Mann-Whitney U test,
    calculates stability metrics, and prepares data for visualization.
    ...
    Returns:
        dict: Results containing baseline analysis results, summary statistics, and plotting data
    """
    ...
    for index, sample_reflectance in iterator:
        ...
        slope, p_val, status_trend, status_mw = np.nan, np.nan, "Skipped", "Skipped"
    ...
    detailed_trends_filename = os.path.join(
        output_dir_baseline,
        'baseline_trends_details.csv'
    )
    trends_df.to_csv(detailed_trends_filename, index=False, float_format='%.6g')
    ...
    num_samples_analyzed = len(trends_df)
    
    if not valid_slopes.empty:
        slope_iqr_val = iqr(valid_slopes)
        abs_slope_threshold = (
            BASELINE_SLOPE_THRESHOLD_FACTOR * slope_iqr_val
            if slope_iqr_val > 1e-9
            else np.percentile(np.abs(valid_slopes), 95)
        )
        strong_slope_count = (np.abs(valid_slopes) > abs_slope_threshold).sum()
        strong_slope_percentage = (
            (strong_slope_count / len(valid_slopes)) * 100
        ...
        summary_metrics['trend_analysis'] = {
            ...
            'strong_slope_threshold_defn': (
                f">{BASELINE_SLOPE_THRESHOLD_FACTOR} * IQR (or 95th Perc if IQR=0)"
            ),
        ...
        logging.info(
            ...
            f"IQR={slope_iqr_val:.3g}, {strong_slope_percentage:.1f}% samples have strong slopes."
        )
    else:
    ...
    if all_sample_derivatives:
        ...
        logging.info(
            f"Baseline Stability Summary: Median Abs Diff={np.median(all_abs_derivatives):.3g}, "
            f"Derivative IQR={iqr(all_sample_derivatives):.3g}"
        )
    else:
        summary_metrics['baseline_stability'] = {'status': 'No derivatives calculated'}
        logging.warning("Could not calculate baseline stability statistics.")
    
    results['summary']['metrics'] = summary_metrics

    # Assess if baseline correction is needed
    ...
    try:
        ...
        if isinstance(trend_metrics, dict):
            ...
            slope_iqr = trend_metrics.get('slope_iqr', 0)
            ...
            factor_slope_mag = abs(med_slope) > abs_thresh if not np.isinf(abs_thresh) else False
            ...
            if isinstance(strong_perc, (int, float)) and strong_perc > BASELINE_SIG_TREND_PERC_THRESHOLD:
                decision_factors['high_perc_strong_slopes'] = {
                    'value': True,
                    'details': f'Percentage={strong_perc:.1f}%'
                }
                correction_needed = True
            else:
                decision_factors['high_perc_strong_slopes'] = {
                    'value': False,
                    'details': f'Percentage={strong_perc}%'
                }
    ...
    logging.info(
        f"Baseline Correction Assessment: Potentially Needed = "
...
def calculate_savitzky_golay_derivatives(spectral_data, wavelengths, output_dir_base):
    """
    Calculate Savitzky-Golay derivatives for spectral data.
    
    This function computes first and second derivatives using the Savitzky-Golay filter,
    calculates summary statistics, and prepares data for visualization.
    ...
    Returns:
        dict: Results containing derivative analysis results, summary statistics, and plotting data
    """
    ...
    output_dir_deriv = os.path.join(output_dir_base, "Derivative_Analysis_SG")
    ...
    if (not isinstance(window, int) or window % 2 == 0 or window < 3 or
            spectral_data.shape[1] < window or not isinstance(poly, int) or poly < 1):
        logging.error(
    ...
    detailed_metrics_filename = os.path.join(
        output_dir_deriv,
        'derivative_sg_details_per_sample.csv'
    )
    metrics_df_sg.to_csv(detailed_metrics_filename, index=False, float_format='%.6g')
    ...
    results['summary'] = {
        ...
        'first_derivative_stats': {
            'overall_median_of_medians': metrics_df_sg['first_deriv_sg_median'].median(),
            'overall_iqr_of_medians': iqr(metrics_df_sg['first_deriv_sg_median'].dropna()),
            'median_of_max_absolute': metrics_df_sg['first_deriv_sg_max_abs'].median()
        },
        'second_derivative_stats': {
            'overall_median_of_medians': metrics_df_sg['second_deriv_sg_median'].median(),
            'overall_iqr_of_medians': iqr(metrics_df_sg['second_deriv_sg_median'].dropna()),
            'median_of_max_absolute': metrics_df_sg['second_deriv_sg_max_abs'].median()
        }
    }
    ...
    spectra_filename = os.path.join(output_dir_deriv, 'derivative_sg_median_spectra.csv')
    derivative_spectra_sg.to_csv(spectra_filename, index=False, float_format='%.6g')
...
#############################################################################
def generate_composite_figure(core_plotting_data, normality_plotting_data, baseline_plotting_data, deriv_plotting_data, output_dir_base):
    ...
    try:
        fig, axes = plt.subplots(3, 3, figsize=(18, 16)) # Adjusted size slightly

        # --- Panel Titles ---
        panel_titles = ['A) Outlier Samples', 'B) Spectral Standard Deviation', 'C) Correlation Heatmap',
                        'D) Shapiro P-value Distribution', 'E) Example Band KDEs', 'F) Distribution of Baseline Slopes',
                        'G) Boxplot of Baseline Slopes', 'H) Median 1st Derivative (Sav-Gol)', 'I) Median 2nd Derivative (Sav-Gol)']

        # --- Row 1: Core Quality ---
        # R1 C1: Outlier Counts
        ax = axes[0, 0]
        outlier_summary = core_plotting_data.get('outlier_summary', {})
        methods_run = core_plotting_data.get('methods_run_outliers', [])
        outlier_plot_data = {'IQR': outlier_summary.get('iqr', {}).get('count', 0),'Mod Z': outlier_summary.get('mod_zscore', {}).get('count', 0),'LOF': outlier_summary.get('lof', {}).get('count', 0)}
        plot_counts = {k: v for k, v in outlier_plot_data.items() if isinstance(v, int) and k in [('IQR' if 'IQR' in methods_run else None), ('Mod Z' if 'Modified Z-Score' in methods_run else None), ('LOF' if 'LOF' in methods_run else None)] if v is not None} # More robust filtering

        if plot_counts:
            colors = plt.get_cmap(PLOT_CMAP)(np.linspace(0.3, 0.7, len(plot_counts)))
            bars = ax.bar(plot_counts.keys(), plot_counts.values(), color=colors, alpha=0.8)
            ax.bar_label(bars, fmt='%d', fontsize=PLOT_LEGEND_FONTSIZE)
            ax.set_ylabel('Number of Samples Flagged', fontsize=PLOT_AXIS_FONTSIZE)
        else: ax.text(0.5, 0.5, 'No outlier counts to plot\n(Check logs)', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[0], weight='bold'); ax.grid(axis='y', linestyle='--', alpha=0.7); ax.tick_params(axis='x', rotation=0, labelsize=PLOT_AXIS_FONTSIZE-1)

        # R1 C2: STD vs Wavelength
        ax = axes[0, 1]
        std_devs = core_plotting_data.get('std_devs')
        if std_devs is not None:
            wavelengths_core = core_plotting_data.get('wavelengths')
            ax.plot(wavelengths_core, std_devs.values, color=plt.get_cmap(PLOT_CMAP)(0.6))
            ax.set_xlabel('Wavelength / Index', fontsize=PLOT_AXIS_FONTSIZE); ax.set_ylabel('Standard Deviation', fontsize=PLOT_AXIS_FONTSIZE)
            ax.grid(axis='both', linestyle='--', alpha=0.6)
        else: ax.text(0.5, 0.5, 'STD data unavailable', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[1], weight='bold')

        # R1 C3: Correlation Heatmap
        ax = axes[0, 2]
        corr_matrix = core_plotting_data.get('corr_matrix_sampled')
        if corr_matrix is not None:
            tick_labels = core_plotting_data.get('corr_tick_labels'); tick_locs = core_plotting_data.get('corr_tick_locs')
            sns.heatmap(corr_matrix, cmap=PLOT_CMAP, vmin=0, vmax=1, ax=ax, cbar=True, cbar_kws={'shrink': .6})
            title_suffix = ' (~100 Bands)' if len(tick_locs) < corr_matrix.shape[0] else ''
            tick_step = max(1, len(tick_locs) // 6); ax.set_xticks(tick_locs[::tick_step]); ax.set_xticklabels([str(lbl).split('_')[-1] for lbl in tick_labels[::tick_step]], rotation=90, fontsize=PLOT_AXIS_FONTSIZE - 2)
            ax.set_yticks(tick_locs[::tick_step]); ax.set_yticklabels([str(lbl).split('_')[-1] for lbl in tick_labels[::tick_step]], rotation=0, fontsize=PLOT_AXIS_FONTSIZE - 2)
        else: ax.text(0.5, 0.5, 'Corr matrix unavailable', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[2] + title_suffix, weight='bold')


        # --- Row 2: Normality & Baseline ---
        # R2 C1: Shapiro p-value Histogram
        ax = axes[1, 0]
        p_values = normality_plotting_data.get('p_values')
        if p_values:
             ax.hist(p_values, bins=50, edgecolor='black', color=plt.get_cmap(PLOT_CMAP)(0.6), alpha=0.8)
             ax.axvline(x=ALPHA, color='r', linestyle='--', label=f'α = {ALPHA}')
             ax.set_xlabel('P-value', fontsize=PLOT_AXIS_FONTSIZE); ax.set_ylabel('Frequency', fontsize=PLOT_AXIS_FONTSIZE); ax.legend(fontsize=PLOT_LEGEND_FONTSIZE)
        else: ax.text(0.5, 0.5, 'No p-values', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[3], weight='bold'); ax.grid(True, linestyle='--', alpha=0.6)

        # R2 C2: Example Band KDEs
        ax = axes[1, 1]
        data_samples = normality_plotting_data.get('spectral_data_samples')
        if data_samples is not None:
            sample_names = normality_plotting_data.get('sample_wavelength_names'); results_df_norm = normality_plotting_data.get('results_df')
            colors = plt.get_cmap(PLOT_CMAP)(np.linspace(0.2, 0.8, len(sample_names)))
            for i, wave in enumerate(sample_names):
                p_val_info = results_df_norm[results_df_norm['wavelength_column'] == wave]['p_value'].iloc[0]; label = f"{str(wave).split('_')[-1]} (p={p_val_info:.1e})" if not pd.isna(p_val_info) else f"{str(wave).split('_')[-1]} (N/A)"
                sns.kdeplot(data=data_samples[wave].dropna(), ax=ax, label=label, color=colors[i], fill=True, alpha=0.4, warn_singular=False)
            ax.set_xlabel('Reflectance / Value', fontsize=PLOT_AXIS_FONTSIZE); ax.set_ylabel('Density', fontsize=PLOT_AXIS_FONTSIZE); ax.legend(title='Band (Shapiro p)', fontsize=PLOT_LEGEND_FONTSIZE)
        else: ax.text(0.5, 0.5, 'No sample data', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[4], weight='bold'); ax.grid(True, linestyle='--', alpha=0.6)

        # R2 C3: Theil-Sen Slope Histogram
        ax = axes[1, 2]
        ...
        if valid_slopes_base is not None and not valid_slopes_base.empty:
             sns.histplot(valid_slopes_base, bins=30, kde=True, ax=ax, color=plt.get_cmap(PLOT_CMAP)(0.7), alpha=0.8)
             med_slope_plot = baseline_plotting_data.get('summary_metrics',{}).get('trend_analysis',{}).get('median_theil_sen_slope', np.nan)
             ax.axvline(med_slope_plot, color='r', linestyle='--', label=f'Median = {med_slope_plot:.2e}')
             ax.set_xlabel('Theil-Sen Slope per Sample', fontsize=PLOT_AXIS_FONTSIZE); ax.set_ylabel('Count', fontsize=PLOT_AXIS_FONTSIZE); ax.legend(fontsize=PLOT_LEGEND_FONTSIZE)
        else: ax.text(0.5, 0.5, 'No slope data', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[5], weight='bold'); ax.grid(True, linestyle='--', alpha=0.6)


        # --- Row 3: Baseline & Derivatives ---
        # R3 C1: Theil-Sen Slope Boxplot
        ax = axes[2, 0]
        if valid_slopes_base is not None and not valid_slopes_base.empty:
            sns.boxplot(y=valid_slopes_base, ax=ax, color=plt.get_cmap(PLOT_CMAP)(0.5), width=0.4, showfliers=True, flierprops={"marker":".", "markersize":3, "markerfacecolor":"gray", "alpha":0.5})
            ax.set_ylabel('Slope per Sample', fontsize=PLOT_AXIS_FONTSIZE); ax.set_xlabel('', fontsize=PLOT_AXIS_FONTSIZE)
            ax.grid(True, axis='y', linestyle='--', alpha=0.6)
        else: ax.text(0.5, 0.5, 'No slope data', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[6], weight='bold')

        # R3 C2: Median 1st Derivative (SG)
        ax = axes[2, 1]
        median_deriv1 = deriv_plotting_data.get('median_first_deriv_sg')
        if median_deriv1 is not None:
            wavelengths_deriv = deriv_plotting_data.get('wavelengths'); d1_25 = deriv_plotting_data.get('first_deriv_sg_25th'); d1_75 = deriv_plotting_data.get('first_deriv_sg_75th')
            ax.plot(wavelengths_deriv, median_deriv1, color=plt.get_cmap(PLOT_CMAP)(0.7), label='Median')
            ax.fill_between(wavelengths_deriv, d1_25, d1_75, color=plt.get_cmap(PLOT_CMAP)(0.7), alpha=0.3, label='IQR')
            ax.set_xlabel('Wavelength / Index', fontsize=PLOT_AXIS_FONTSIZE); ax.set_ylabel('1st Derivative', fontsize=PLOT_AXIS_FONTSIZE); ax.legend(fontsize=PLOT_LEGEND_FONTSIZE)
        else: ax.text(0.5, 0.5, 'No SG deriv data', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[7], weight='bold'); ax.grid(True, linestyle='--', alpha=0.6)

        # R3 C3: Median 2nd Derivative (SG)
        ax = axes[2, 2]
        median_deriv2 = deriv_plotting_data.get('median_second_deriv_sg')
        if median_deriv2 is not None:
            wavelengths_deriv = deriv_plotting_data.get('wavelengths'); d2_25 = deriv_plotting_data.get('second_deriv_sg_25th'); d2_75 = deriv_plotting_data.get('second_deriv_sg_75th')
            ax.plot(wavelengths_deriv, median_deriv2, color=plt.get_cmap(PLOT_CMAP)(0.5), label='Median')
            ax.fill_between(wavelengths_deriv, d2_25, d2_75, color=plt.get_cmap(PLOT_CMAP)(0.5), alpha=0.3, label='IQR')
            ax.set_xlabel('Wavelength / Index', fontsize=PLOT_AXIS_FONTSIZE); ax.set_ylabel('2nd Derivative', fontsize=PLOT_AXIS_FONTSIZE); ax.legend(fontsize=PLOT_LEGEND_FONTSIZE)
        else: ax.text(0.5, 0.5, 'No SG deriv data', ha='center', va='center', transform=ax.transAxes, fontsize=PLOT_AXIS_FONTSIZE)
        ax.set_title(panel_titles[8], weight='bold'); ax.grid(True, linestyle='--', alpha=0.6)

        # --- Final Adjustments ---
        fig.suptitle('Comprehensive Data Quality Assessment Summary', fontsize=PLOT_TITLE_FONTSIZE + 4, y=1.0, weight='bold') # Made bold
        plt.tight_layout(rect=[0, 0.02, 1, 0.98]) # Adjust layout rect to prevent title overlap

        composite_plot_filename = os.path.join(output_dir_plots, 'composite_quality_summary.png')
...
def run_data_quality_pipeline(input_file, spec_start_col, output_dir):
    ...
    logger = logging.getLogger()
    logger.info(f"=== Starting Data Quality Pipeline ===")
    logger.info(f"Input File: {input_file}"); logger.info(f"Output Directory: {output_dir}")
    logger.info(f"Config Flags: LOF={RUN_LOF_OUTLIERS}, Normality={RUN_NORMALITY_TESTS}, Baseline={RUN_BASELINE_ASSESSMENT}, Derivative={RUN_DERIVATIVE_ANALYSIS}")

    pipeline_results = {"pipeline_start_time": datetime.now().isoformat(), "input_file": input_file, "output_directory": output_dir, "steps_run": [], "data_preparation": {}, "core_quality": {}, "normality": {}, "baseline": {}, "derivatives_sg": {}, "summary_plots": {}}
    composite_plot_data = {}

    try:
        # --- Step 1 ---
        metadata, spectral_data, wavelengths, prep_info = load_and_prepare_data(input_file, spec_start_col)
        pipeline_results["data_preparation"] = prep_info; pipeline_results["steps_run"].append("1_Load_Prepare")
        composite_plot_data['spectral_data'] = spectral_data # Pass along if needed later

        # --- Step 2 ---
        core_results = perform_core_quality_assessment(spectral_data, wavelengths, output_dir)
        pipeline_results["core_quality"] = core_results; pipeline_results["steps_run"].append("2_Core_Quality")
        ...
        pipeline_results["normality"] = normality_results; pipeline_results["steps_run"].append("3_Normality")
        composite_plot_data['normality'] = normality_results.get('plotting_data', {})

        # --- Step 4 ---
        baseline_results = perform_robust_baseline_assessment(spectral_data, output_dir)
        pipeline_results["baseline"] = baseline_results; pipeline_results["steps_run"].append("4_Baseline")
        composite_plot_data['baseline'] = baseline_results.get('plotting_data', {})

        # --- Step 5 ---
        derivative_results = calculate_savitzky_golay_derivatives(spectral_data, wavelengths, output_dir)
        pipeline_results["derivatives_sg"] = derivative_results; pipeline_results["steps_run"].append("5_Derivatives_SG")
        composite_plot_data['derivatives_sg'] = derivative_results.get('plotting_data', {})

        # --- Step 6 ---
        composite_filename = generate_composite_figure(
            core_plotting_data=composite_plot_data.get('core',{}), normality_plotting_data=composite_plot_data.get('normality',{}),
            baseline_plotting_data=composite_plot_data.get('baseline',{}), deriv_plotting_data=composite_plot_data.get('derivatives_sg',{}),
            output_dir_base=output_dir
        )
        if composite_filename: pipeline_results["summary_plots"]["composite_figure"] = composite_filename
        pipeline_results["steps_run"].append("6_Composite_Plot")

        logger.info("=== Data Quality Pipeline Finished Successfully ===")

    except Exception as e: logger.exception("FATAL ERROR during pipeline execution."); pipeline_results["pipeline_error"] = str(e)

    finally:
        pipeline_results["pipeline_end_time"] = datetime.now().isoformat(); report_filename = os.path.join(output_dir, "pipeline_summary_report.json")
        save_report_to_json(pipeline_results, report_filename); logger.info(f"Pipeline summary report saved to {report_filename}")
        # Print Summary
        print("\n" + "="*60); print("Pipeline Execution Summary:"); print(f"  Status: {'Completed' if 'pipeline_error' not in pipeline_results else 'Failed'}")
        if 'pipeline_error' in pipeline_results: print(f"  Error: {pipeline_results['pipeline_error']}")
        print(f"  Steps Run: {pipeline_results['steps_run']}")
        core_summary = pipeline_results.get('core_quality', {}); outlier_count = core_summary.get('outliers', {}).get('combined_unique_outliers', {}).get('count', 'N/A'); median_std = core_summary.get('basic_metrics', {}).get('median_std_dev', 'N/A')
        print(f"\n  Core Quality:"); print(f"    - Potential Outliers Found (Combined): {outlier_count}"); print(f"    - Median Spectral Std Dev: {median_std:.4g}" if isinstance(median_std, (int, float)) else f"Median Spectral Std Dev: {median_std}")
        norm_summary = pipeline_results.get('normality', {}).get('summary', {}); perc_normal = norm_summary.get('percent_normal_of_tested', 'N/A')
        print(f"\n  Normality (Shapiro-Wilk):"); print(f"    - % Normal Bands (of Tested): {perc_normal:.1f}%" if isinstance(perc_normal, (int, float)) else f"% Normal Bands (of Tested): {perc_normal}")
        base_summary = pipeline_results.get('baseline', {}).get('summary', {}); base_needed = base_summary.get('assessment', {}).get('correction_potentially_needed', 'N/A')
        print(f"\n  Baseline Assessment:"); print(f"    - Baseline Correction Potentially Needed: {base_needed}")
        if pipeline_results.get("summary_plots",{}).get("composite_figure"): print(f"\n  Composite Summary Plot Generated: {pipeline_results['summary_plots']['composite_figure']}")
        print("="*60)
...
if __name__ == "__main__":
    ...
    run_data_quality_pipeline(input_file=INPUT_FILE_HYPER, spec_start_col=SPECTRAL_DATA_START_COL, output_dir=OUTPUT_DIR)
    print("\n==============================================")
    print(f"Pipeline execution finished.")
    print(f"Check the output directory for results:\n{OUTPUT_DIR}")
    print(f"Check the main log file and JSON report for details.")
    print("==============================================")