...
TEXT = {
    ...
    'main_title': 24,         # Figure Title
    'panel_label': 21,        # Panel Labels
    'panel_title': 19,        # Title for each subplot/panel
    'axis_label': 19,         # X and Y axis labels
    'tick_label': 18,         # Axis tick numbers/text
    'legend_title': 21,       # Title of the legend box
    'legend_text': 18,        # Text for individual legend items
    'annotation': 17,         # Text annotations within the plot area
    'caption': 17,            # Figure caption text
    'table_header': 17,       # Text in table headers
    'table_cell': 17,         # Text in table cells
    'tissue_label': 16        # Tissue labels (Leaf, Root)
}
...
LAYOUT = {
    ...
    'top_margin': 0.90,          # Top margin for layout
...
def clean_metadata_columns(df):
    ...
    # Rename original 'Day' column if it exists
        ...
        df['Time point'] = pd.to_numeric(df['Time point'], errors='coerce')
        df = df.dropna(subset=['Time point'])
        df['Time point'] = df['Time point'].astype(int)
...
def prepare_network_data(df, tissue, n_top_connections=40, min_attention=0.01):
    ...
    # Filter for stress condition (T1)
...
def create_network_visualization(fig, ax, data, geno, tissue, node_importances, pos=None):
    ...
    for molecular_feature in data['all_molecular_feature']:
        ...
        G.add_node(molecular_feature, bipartite=1, type='molecular_feature', category=category, color=color, importance=importance)
    ...
    if pos is None:
        ...
        molecular_feature_nodes = {n for n, d in G.nodes(data=True) if d['type'] == 'molecular_feature'}
        ...
        molecular_feature_matrix = np.zeros((len(molecular_feature_nodes), len(molecular_feature_nodes)))
    ...
    molecular_feature_nodes = {n: G.nodes[n]['importance'] for n, d in G.nodes(data=True) if d['type'] == 'molecular_feature'}
    
    # Sort by importance
    top_spec = sorted(spec_nodes.items(), key=lambda x: x[1], reverse=True)[:3]
    top_molecular_feature = sorted(molecular_feature_nodes.items(), key=lambda x: x[1], reverse=True)[:3]
    ...
    ax.text(0.7, -0.95, "Molecular Features", ha='center', fontsize=TEXT['axis_label'])
    ...
    for tissue, overall_df, cond_df, view_df in [
        ...
        if overall_df is not None and not overall_df.empty and 'Spectral_Feature' in overall_df.columns:
            top_100_pairs = set(zip(overall_df['Spectral_Feature'].head(100),
                                    overall_df['Molecular_Feature'].head(100)))
        else:
        ...
        if cond_df is not None and not cond_df.empty:
            cond_t1d3 = cond_df[(cond_df['Time point'] == 3) & (cond_df['Treatment'] == 'T1')]
        else:
            cond_t1d3 = pd.DataFrame()
            
        if view_df is not None and not view_df.empty:
            view_t1d3 = view_df[(view_df['Time point'] == 3) & (view_df['Treatment'] == 'T1')]
        else:
                   ...
                   'Mean_Attention_S2M_Group_AvgHeads' in geno_cond_t1d3.columns:
                    ...
                    geno_top_pairs_cond = geno_cond_t1d3[
                        geno_cond_t1d3.apply(
                            lambda row: (row['Spectral_Feature'], row['Molecular_Feature']) in top_100_pairs,
                            axis=1
                        )
                    ]
...
def create_network_stats_visualization(fig, network_metrics_df, ax_row_idx):
        ...
        if len(tissue_data) == 2:  # Ensure we have both G1 and G2 data
            ...
            for metric in metrics_to_plot:
                ...
                if pd.notna(g1_val) and pd.notna(g2_val) and g2_val != 0:
                    pct_diff = ((g1_val - g2_val) / abs(g2_val)) * 100
    ...
    plot_metrics_order = [m for m in metrics_to_plot if m in plot_df['Metric'].unique()]
    ...
    if metric_data_list:
        ...
        for tissue_idx, tissue in enumerate(['Leaf', 'Root']):
            base_position = tissue_idx * (len(plot_metrics_order) + 1)
            ...
            for metric_idx, metric in enumerate(plot_metrics_order):
                ...
                if not g1_data.empty and not g2_data.empty:
                    ...
                    if pd.notna(g1_val) and pd.notna(g2_val) and pd.notna(pct_diff):
                        # Position closer to the bars
                        max_height = max(g1_val, g2_val)
                        if max_height > 0.04:
                            y_pos = max_height * 1.12
                        else:
                            y_pos = max_height * 1.18
...
def rename_legacy_columns(df):
    """Rename legacy columns to match current standard."""
    ...
    # Rename values within the Molecular_Feature column
...
if __name__ == "__main__":
    ...
    # Create legends with all elements moved to the right-hand side
    ...
    # Move the colorbar to top with larger text size
