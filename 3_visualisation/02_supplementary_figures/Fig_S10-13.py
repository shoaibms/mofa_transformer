...
class VisualizationSuite:
    ...
    def load_data(self):
        ...
        if 'Row_names' in self.common_metadata:
            self.common_metadata.remove('Row_names')
    ...
    def setup_plotting_style(self):
        ...
        plt.rcParams['font.size'] = self.FONTS['tick_label']
        ...
        self.plot_colors = {
            'original': self.COLORS.get('G1', '#3182bd'),
            'augmented': self.COLORS.get('T1', '#fe9929'),
        ...
        self.discrete_cmap = plt.cm.tab10
    ...
    def factor_specific_augmentation_effect(self):
        ...
        if not common_factors:
            ...
            spectral_cat_cols = [col for col in self.spectral_metadata_cols
                                ...
                                or (self.spectral_original[col].nunique() < 10 and self.spectral_original[col].nunique() > 1)]
        ...
        fig = plt.figure(figsize=(18, 7 * n_factors))
        gs = gridspec.GridSpec(n_factors, 2, figure=fig, hspace=0.5, wspace=0.3)
        ...
        for i, factor in enumerate(common_factors):
            ...
            row_top = 1.0 - (i / n_factors) * (1 - gs.hspace / (7 * n_factors / 18))
    ...
    def _plot_spectral_factor_effect(self, factor, ax):
        ...
        level_cmap = plt.cm.get_cmap('tab10', len(factor_levels))
        ...
        if factor in augmented_only.columns:
            augmented_only[factor] = augmented_only[factor].astype(self.spectral_original[factor].dtype)
        ...
        if not augmented_only.empty:
            ...
            if divergence_metrics:
                ...
                ax.text(0.02, 0.98, divergence_text, transform=ax.transAxes,
                        ...
                        fontsize=self.FONTS['annotation'] - 2,
        ...
        ax.legend(loc='lower right', fontsize=self.FONTS['legend_text'] - 4, ncol=2)
    ...
    def _plot_molecular_features_factor_effect(self, factor, ax):
        ...
        if factor in augmented_only.columns:
             augmented_only[factor] = augmented_only[factor].astype(self.metabolite_original[factor].dtype)
        ...
        pivot_data = pivot_data.dropna(axis=1, how='all').dropna(axis=0, how='all')
        ...
        if pivot_data.empty:
            ...
            ax.figure.axes[-1].yaxis.label.set_size(self.FONTS['axis_label'])
            ax.tick_params(axis='x', labelsize=self.FONTS['tick_label'] - 6)
                ...
                for level in factor_levels:
                    if f"{level} (O)" in pivot_data.index and f"{level} (A)" in pivot_data.index:
                        ...
                        try:
                            ...
                            bins = min(10, max(3, len(orig_values_aligned) // 2))
    ...
    def _identify_top_metabolites(self, factor, n_top=20):
        ...
        for metabolite in self.metabolite_cols:
            ...
            level_values = [metabolite_data[metabolite_data[factor] == level][metabolite].dropna().values
                            for level in factor_levels]
            level_values = [vals for vals in level_values if len(vals) >= 2]
    ...
    def _create_spectral_signature_figure(self):
        ...
        if len(aug_spectra) > 0:
            ...
            ax.plot(wavelength_values, aug_median[wavelength_indices],
                    color=self.plot_colors['augmented'], linestyle='-',
        ...
        for region_name, (start, end) in regions.items():
            ax.axvspan(start, end, alpha=0.08, color=self.COLORS['Grid'])
        ...
        if len(aug_spectra) > 0:
            ...
            for wl in key_wavelengths:
                ...
                if len(orig_values) >= 3 and len(aug_values) >= 3:
                    try:
    ...
    def _identify_differential_metabolites(self, n_top=20):
        ...
        for metabolite in self.metabolite_cols:
            ...
            try:
                ...
                diff_metrics[metabolite] = abs(effect_size)
    ...
    def create_parallel_coordinate_plot(self):
        ...
        for i, method in enumerate(spec_methods):
            method_colors[method] = self.COLORS.get(method, color_map_spec(i+1))
        for i, method in enumerate(mol_methods):
            method_colors[method] = self.COLORS.get(method, color_map_mol(i+1))
    ...
    def augmentation_quality_heatmaps(self):
        """Generate augmentation quality heatmaps."""
        print("Generating augmentation quality heatmaps...")
        pass
    ...
    def _create_method_count_panel(self, ax):
        """Create panel showing sample counts by method and data type."""
            ...
            for p in ax.patches:
                ...
                if height > 0:
                    ax.text(p.get_x() + p.get_width() / 2., height + ax.get_ylim()[1]*0.01, f'{int(height)}',
                            ha='center', va='bottom', fontsize=self.FONTS['annotation'] - 4)
    ...
    def _create_metric_bar_panel(self, ax, metric_name, metric_func_suffix):
        """Base function for creating distribution, structure, and biological bar chart panels."""
            ...
            for p in ax.patches:
                ...
                if height > 0:
                    ax.text(p.get_x() + p.get_width() / 2., height + 0.01, f'{height:.2f}',
                            ha='center', va='bottom', fontsize=self.FONTS['annotation'] - 5)
    ...
    def _create_factor_scores_panel(self, ax):
        ...
        if not factor_scores_data:
            ...
            if pivot_data.empty:
                ...
                ax.figure.axes[-1].yaxis.label.set_size(self.FONTS['axis_label']-4)
                ax.tick_params(axis='y', rotation=0, labelsize=self.FONTS['tick_label'] - 4)
                ax.tick_params(axis='x', labelsize=self.FONTS['tick_label'] - 4)
    ...
    def _create_method_comparison_figure(self):
        ...
        if n_cols == 0:
            ...
            gs = gridspec.GridSpec(1, n_cols, figure=fig, wspace=0.4)
            col_idx = 0
            if spectral_metrics:
                ax1 = fig.add_subplot(gs[0, col_idx], polar=True)
                self._create_radar_chart(ax1, categories, spectral_metrics, "Spectral Methods")
                col_idx += 1
            if molecular_features_metrics:
                 ax2 = fig.add_subplot(gs[0, col_idx], polar=True)
                 self._create_radar_chart(ax2, categories, molecular_features_metrics, "Molecular Features Methods")
    ...
    def _create_radar_chart(self, ax, categories, metrics_dict, title):
        ...
        ax.set_xticklabels(categories, fontsize=self.FONTS['tick_label']-2, color=self.COLORS['Text_Dark'])
    ...
    def _calculate_factor_score(self, factor):
        ...
        for method in self.spectral_methods:
            score = self._treatment_metric('spectral', method) if factor=='Treatment' else \
                    ...
                    self._biological_metric('spectral', method)
    ...
    def run_all_visualizations(self):
        ...
        viz_methods = [
            ...
            ("Augmentation Quality Heatmaps", self.augmentation_quality_heatmaps),
    ...
    def _identify_differential_molecular_features(self, n_top=20):
        ...
        for molecular_feature in self.molecular_features_cols:
            ...
            try:
                ...
                # Rank-Biserial Correlation (effect size for MWU)
                effect_size = 1 - (2 * u_stat) / (n1 * n2)
                diff_metrics[molecular_feature] = abs(effect_size)
