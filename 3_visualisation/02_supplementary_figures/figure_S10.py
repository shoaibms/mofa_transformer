"""
This script generates and saves confusion matrices for multiple classification tasks
(Genotype, Treatment, Day) across different tissues (Leaf, Root).
It reads prediction data from CSV files, processes it, and uses matplotlib and
seaborn to create a grid of confusion matrices, visualizing the performance of
classification models. The resulting plot is saved as a PNG image.
Error handling is included to output an image with an error message if the
generation fails.
"""
import pandas as pd
import numpy as np
