...
2. Core quality assessment (outlier detection, basic metrics).
3. Normality assessment (Shapiro-Wilk test).
4. Baseline and stability assessment.
5. Derivative analysis (Savitzky-Golay).
6. Generation of a composite summary figure.

The pipeline produces a JSON report and visualizations in the specified
output directory.
"""
...
from scipy.stats import iqr, mannwhitneyu, theilslopes
...
# Statistical and Algorithm Constants
ALPHA = 0.05
...
def save_report_to_json(report_dict, filename):
    """Save the analysis report to a JSON file, handling numpy types."""
    try:
...
def get_iterator(iterable, desc="Processing"):
    """Return a tqdm iterator if available, otherwise the original iterable."""
...
def load_and_prepare_data(file_path, spec_start_col):
    """
    Load data, separate metadata and spectral data, handle NaNs, and extract wavelengths.
    """
    ...
    try:
        ...
        logging.error(f"Critical: Input file not found at {file_path}")
        raise
    except ValueError as ve:
        logging.error(f"Critical: ValueError during data preparation: {ve}")
        raise
    except Exception as e:
        logging.error(
            "Critical: An unexpected error occurred during data "
...
def perform_core_quality_assessment(spectral_data, wavelengths, output_dir_base):
    """
    Perform outlier detection and calculate basic metrics.
    """
...
def perform_normality_assessment(spectral_data, output_dir_base):
    """
    Test normality per band (Shapiro-Wilk) and return summary/plotting data.
    """
...
def perform_robust_baseline_assessment(spectral_data, output_dir_base):
    """
    Assess baseline trends and stability using Theil-Sen estimator and derivatives.
    """
...
def calculate_savitzky_golay_derivatives(spectral_data, wavelengths, output_dir_base):
    """
    Calculate Savitzky-Golay derivatives (1st and 2nd order) and return plotting data.
    """
...
def generate_composite_figure(core_plotting_data, normality_plotting_data,
                              ...
                              output_dir_base):
    """
    Generate a single 3x3 composite figure summarizing quality checks.
    """
...
def run_data_quality_pipeline(input_file, spec_start_col, output_dir):
    ...
    try:
        ...
        logger.exception("Critical Error during pipeline execution.")
        ...
        if pipeline_results.get("summary_plots", {}).get("composite_figure"):
            print(
