...
# Pattern to detect augmented samples (anchored at end of Row_names)
AUG_PATTERN = re.compile(r"(?:_GP|_WARP|_SCALE|_NOISE|_ADD|_MULT|_MIX)$")


def filter_augmented_for_export(metadata_df: pd.DataFrame, attention_dict: dict):
    """
    Filter augmented samples from test metadata and attention tensors.
    Ensures interpretability outputs contain original samples only.
    
    Augmentation was used for training robustness; interpretability
    analysis is performed on original held-out samples only.
    
    Args:
        metadata_df: Test metadata DataFrame with Row_names as index
        attention_dict: Dict with '1_to_2' (and optionally '2_to_1') attention tensors
        
    Returns:
        Tuple of (filtered_metadata_df, filtered_attention_dict)
    """
    if metadata_df is None or metadata_df.empty:
        logger.warning("filter_augmented_for_export: metadata_df is None or empty.")
        return metadata_df, attention_dict
    
    # Identify augmented samples by index
    row_ids = metadata_df.index.astype(str)
    mask_augmented = pd.Series(row_ids).str.contains(AUG_PATTERN).values
    
    n_total = len(metadata_df)
    n_aug = mask_augmented.sum()
    n_orig = n_total - n_aug
    
    logger.info(f"Augmentation filter for export: {n_total} test samples -> {n_orig} original ({n_aug} augmented removed)")
    
    if n_aug == 0:
        logger.info("No augmented samples found in test set. No filtering needed.")
        return metadata_df, attention_dict
    
    # Filter metadata
    mask_original = ~mask_augmented
    metadata_filtered = metadata_df.loc[mask_original].copy()
    
    # Filter attention tensors
    if attention_dict is not None and '1_to_2' in attention_dict:
        attn_s2m = attention_dict['1_to_2']
        
        # Handle both torch.Tensor and numpy array
        if isinstance(attn_s2m, torch.Tensor):
            attn_s2m_filtered = attn_s2m[mask_original]
        else:
            attn_s2m_filtered = attn_s2m[mask_original]
        
        attention_filtered = {'1_to_2': attn_s2m_filtered}
        
        # Also filter M2S if present
        if '2_to_1' in attention_dict and attention_dict['2_to_1'] is not None:
            attn_m2s = attention_dict['2_to_1']
            if isinstance(attn_m2s, torch.Tensor):
                attention_filtered['2_to_1'] = attn_m2s[mask_original]
            else:
                attention_filtered['2_to_1'] = attn_m2s[mask_original]
        
        logger.info(f"Attention tensor S2M filtered: {attn_s2m.shape} -> {attn_s2m_filtered.shape}")
        return metadata_filtered, attention_filtered
    
    return metadata_filtered, attention_dict
...
def setup_logging(log_dir: str, script_name: str, version: str) -> logging.Logger:
    """
    Set up logging with:
      1) Verbose file log (unchanged)
      2) Console log (unchanged)
      3) Compact summary file log (filtered; intended for quick review)
    """
    os.makedirs(log_dir, exist_ok=True)
    ts = datetime.now()
    log_filename = f"{script_name}_{version}_{ts:%Y%m%d_%H%M%S}.log"
    log_filepath = os.path.join(log_dir, log_filename)
    summary_filename = f"{script_name}_{version}_{ts:%Y%m%d_%H%M%S}.summary.log"
    ...
    class _CompactSummaryFilter(logging.Filter):
        """
        Drop high-frequency / noisy INFO lines from the compact summary log.
        WARNING/ERROR always pass.
        """
        _DROP_SUBSTRINGS = (
            "Evaluate Batch",
            "Raw S->M attention shape",
            "Raw M->S attention shape",
            "Evaluate: Attempting to concatenate",
            "Validated attention tensor shape for HDF5",
            "Saving raw tensors and features to HDF5",
            "Saving attention tensors",
            "Saving feature names",
            "Opened HDF5 file for writing",
            "Saved attention_spec_to_metab",
            "Saved attention_metab_to_spec",
            "Saved feature name datasets",
        )
    ...
    summary_handler = logging.FileHandler(summary_filepath, encoding='utf-8')
    summary_handler.setLevel(logging.INFO)
    summary_handler.setFormatter(summary_formatter)
    summary_handler.addFilter(_CompactSummaryFilter())
    ...
    # Keep this line unchanged to preserve terminal output.
...
def load_and_preprocess_data(config: dict) -> tuple:
    ...
    # ==========================================================================
    # Train/Val/Test Split (Base-ID Aware, Stratified on ORIGINALS ONLY)
    # ==========================================================================
    # Critical: split is performed on original biological samples only (base_id),
    # then augmentations are assigned to the same partition as their base sample.
    # Test set contains ORIGINALS ONLY (no augmentations) to ensure unbiased 
    # evaluation and eliminate leakage from augmented variants of the same plant.
    #
    # This enforces the manuscript claim: 
    # "test set consisted exclusively of original, non-augmented samples"
    # ==========================================================================

    # 1) Identify originals vs augmentations
    row_ids = full_metadata_df.index.astype(str)
    base_ids = row_ids.str.replace(AUG_PATTERN, '', regex=True)
    full_metadata_df['base_id'] = base_ids.values
    full_metadata_df['is_original'] = (row_ids.values == base_ids.values)

    n_total = len(full_metadata_df)
    n_orig = int(full_metadata_df['is_original'].sum())
    n_aug = n_total - n_orig
    logger.info(f"Dataset composition: total={n_total}, originals={n_orig}, augmented={n_aug}")

    # 2) Build stratification key on ORIGINALS ONLY
    originals_df = full_metadata_df[full_metadata_df['is_original']].copy()

    try:
        originals_df['stratify_key'] = originals_df[target_cols[0]].astype(str)
        for col in target_cols[1:]:
            originals_df['stratify_key'] += '_' + originals_df[col].astype(str)
    except KeyError as e:
        logger.error(f"Stratification key column missing: {e}")
        raise

    orig_indices = originals_df.index
    orig_stratify = originals_df['stratify_key']

    # 3) Split ORIGINALS into train/val/test (using config consistently)
    try:
        orig_train_idx, orig_temp_idx = train_test_split(
            orig_indices,
            test_size=(config['VAL_SIZE'] + config['TEST_SIZE']),
            random_state=config['RANDOM_SEED'],
            stratify=orig_stratify
        )
    except ValueError as e:
        logger.warning(f"Full stratification failed ({e}). Falling back to '{target_cols[0]}' only.")
        orig_train_idx, orig_temp_idx = train_test_split(
            orig_indices,
            test_size=(config['VAL_SIZE'] + config['TEST_SIZE']),
            random_state=config['RANDOM_SEED'],
            stratify=originals_df[target_cols[0]].astype(str)
        )

    relative_test_size = config['TEST_SIZE'] / (config['VAL_SIZE'] + config['TEST_SIZE'])
    temp_stratify_values = originals_df.loc[orig_temp_idx, 'stratify_key']

    try:
        orig_val_idx, orig_test_idx = train_test_split(
            orig_temp_idx,
        ...
        logger.warning(f"Val/Test stratification failed ({e}). Falling back to '{target_cols[0]}' only.")
        orig_val_idx, orig_test_idx = train_test_split(
            orig_temp_idx,
            test_size=relative_test_size,
            random_state=config['RANDOM_SEED'],
            stratify=originals_df.loc[orig_temp_idx, target_cols[0]].astype(str)
        )

    # 4) Assign augmentations to same partition as their base_id
    train_base = set(map(str, orig_train_idx))
    val_base = set(map(str, orig_val_idx))
    test_base = set(map(str, orig_test_idx))

    # Train/Val: originals + augmentations whose base_id is in that split
    # Test: ORIGINALS ONLY (no augmentations)
    train_idx = full_metadata_df.index[full_metadata_df['base_id'].isin(train_base)]
    val_idx = full_metadata_df.index[full_metadata_df['base_id'].isin(val_base)]
    test_idx = full_metadata_df.index[
        (full_metadata_df['base_id'].isin(test_base)) & 
        (full_metadata_df['is_original'])
    ]

    logger.info(
        f"Split sizes: Train={len(train_idx)} (originals={len(orig_train_idx)}), "
        f"Val={len(val_idx)} (originals={len(orig_val_idx)}), "
        f"Test={len(test_idx)} (originals_only=True)"
    )

    # Verify no augmented samples in test
    if not full_metadata_df.loc[test_idx, 'is_original'].all():
        raise RuntimeError("LEAKAGE DETECTED: augmented samples in test_idx!")

    # ==========================================================================
    # End of Base-ID Split Block
    # ==========================================================================
    ...
    return (train_loader, val_loader, test_loader,
            ...
            test_metadata_df,
            full_metadata_df)  # Added for ALL_ORIG export
...
def main():
    ...
    # --- Filter augmented samples for interpretability export ---
    # Augmentation used for training robustness only.
    # All attention-based interpretability analysis uses original samples.
    logger.info("--- Filtering Augmented Samples for Interpretability Export ---")
    test_metadata_aligned, test_attention = filter_augmented_for_export(
        test_metadata_aligned, test_attention
    )
    if test_metadata_aligned is not None:
        logger.info(f"Post-filter: {len(test_metadata_aligned)} original samples for interpretability analysis")
    # --- End augmentation filter ---
    ...
    if validation_passed:
        ...
        # ==================================================================
        # Export attention for ALL ORIGINAL (non-augmented) samples
        # ==================================================================
        # This is for Figure 4 biology/interpretability analysis.
        # Uses all originals from train+val+test to maximize sample size.
        # 
        # IMPORTANT: We reuse the existing analyze_attention_feature_level()
        # function to ensure consistent output format.
        # ==================================================================
        if EXPORT_ALL_ORIGINALS_ATTENTION:
            logger.info("="*60)
            logger.info("Exporting ALL_ORIGINALS Attention (for Figure 4)")
            logger.info("="*60)
            
            try:
                # Helper to filter to originals only
                def _only_original(df: pd.DataFrame) -> pd.DataFrame:
                    idx = df.index.astype(str)
                    mask = ~idx.str.contains(AUG_PATTERN)
                    return df.loc[mask]

                # Get originals from each split
                X_train_spec_orig = _only_original(X_train_spec_scaled_df)
                X_val_spec_orig = _only_original(X_val_spec_scaled_df)
                X_train_metab_orig = _only_original(X_train_metab_scaled_df)
                X_val_metab_orig = _only_original(X_val_metab_scaled_df)
                y_train_orig = _only_original(y_train_df)
                y_val_orig = _only_original(y_val_df)

                # Combine all originals (test is already originals-only after clean split)
                X_all_spec_orig = pd.concat([X_train_spec_orig, X_val_spec_orig, X_test_spec_scaled_df], axis=0)
                X_all_metab_orig = pd.concat([X_train_metab_orig, X_val_metab_orig, X_test_metab_scaled_df], axis=0)
                y_all_orig = pd.concat([y_train_orig, y_val_orig, y_test_df], axis=0)

                logger.info(f"ALL_ORIG dataset: {len(X_all_spec_orig)} samples")

                # Get metadata for all originals
                meta_all_orig = full_metadata_df.loc[X_all_spec_orig.index].copy()

                # Create dataset and loader
                all_orig_dataset = PlantOmicsDataset(X_all_spec_orig, X_all_metab_orig, y_all_orig)
                all_orig_loader = DataLoader(
                    all_orig_dataset,
                ...
                # Extract attention using the same evaluate() function
                logger.info("Extracting attention for all originals...")
                _, _, all_preds_df, _, all_attention = evaluate(
                    model, all_orig_loader, criterion, DEVICE, TARGET_COLS
                )

                # Align metadata with predictions
                meta_aligned = meta_all_orig.reindex(all_preds_df.index)

                # Double-check no augmented samples (should be clean, but verify)
                idx_check = meta_aligned.index.astype(str)
                n_aug_check = idx_check.str.contains(AUG_PATTERN).sum()
                if n_aug_check > 0:
                    logger.warning(f"Found {n_aug_check} augmented in ALL_ORIG - filtering...")
                    mask_orig = ~idx_check.str.contains(AUG_PATTERN)
                    meta_aligned = meta_aligned.loc[mask_orig]
                    if all_attention.get('1_to_2') is not None:
                        all_attention['1_to_2'] = all_attention['1_to_2'][mask_orig.values]
                    if all_attention.get('2_to_1') is not None:
                        all_attention['2_to_1'] = all_attention['2_to_1'][mask_orig.values]

                logger.info(f"Final ALL_ORIG count: {len(meta_aligned)} samples")

                # Ensure index name for consistency
                if meta_aligned.index.name != 'Row_names':
                    meta_aligned = meta_aligned.rename_axis('Row_names')

                # ==============================================================
                # REUSE EXISTING EXPORT FUNCTION
                # ==============================================================
                # Use analyze_attention_feature_level() with modified pairing name
                # This ensures consistent output format with test-only exports
                # ==============================================================
                
                all_orig_pairing = f"{ANALYSIS_PAIRING}_ALL_ORIG"
                
                # Save metadata (same format as test export)
                meta_out_path = os.path.join(
                    RESULTS_SUBDIR, 
                    f"raw_attention_metadata_{all_orig_pairing}.csv"
                )
                meta_aligned.to_csv(meta_out_path, index=True, index_label='Row_names')
                logger.info(f"ALL_ORIG metadata saved: {meta_out_path}")
                
                # Try feather as well for faster loading
                try:
                    meta_out_feather = os.path.join(
                        RESULTS_SUBDIR,
                        f"raw_attention_metadata_{all_orig_pairing}.feather"
                    )
                    meta_aligned.reset_index().to_feather(meta_out_feather)
                    logger.info(f"ALL_ORIG metadata saved (Feather): {meta_out_feather}")
                except Exception:
                    pass  # Feather is optional
                
                # Save attention HDF5 using existing pattern
                attn_out_h5 = os.path.join(
                    RESULTS_SUBDIR,
                    f"raw_attention_data_{all_orig_pairing}.h5"
                )
                
                with h5py.File(attn_out_h5, 'w') as f:
                    # S->M attention
                    if all_attention.get('1_to_2') is not None:
                        attn_s2m = all_attention['1_to_2']
                        if isinstance(attn_s2m, torch.Tensor):
                            attn_s2m = attn_s2m.detach().cpu().numpy()
                        f.create_dataset('attention_spec_to_metab', data=attn_s2m, compression='gzip')
                        logger.info(f"  Saved attention_spec_to_metab: {attn_s2m.shape}")
                    
                    # M->S attention
                    if all_attention.get('2_to_1') is not None:
                        attn_m2s = all_attention['2_to_1']
                        if isinstance(attn_m2s, torch.Tensor):
                            attn_m2s = attn_m2s.detach().cpu().numpy()
                        f.create_dataset('attention_metab_to_spec', data=attn_m2s, compression='gzip')
                        logger.info(f"  Saved attention_metab_to_spec: {attn_m2s.shape}")
                    
                    # Feature names (same as test export)
                    f.create_dataset('spectral_feature_names', 
                                    data=np.array(spectral_feature_names, dtype='S'))
                    f.create_dataset('metabolite_feature_names', 
                                    data=np.array(metabolite_feature_names, dtype='S'))
                
                logger.info(f"ALL_ORIG attention saved: {attn_out_h5}")
                logger.info("ALL_ORIGINALS export complete.")

            except Exception as e_all:
                logger.error(f"Failed to export ALL_ORIGINALS attention: {e_all}", exc_info=True)
        
        # ==================================================================
        # End ALL_ORIGINALS export
        # ==================================================================
