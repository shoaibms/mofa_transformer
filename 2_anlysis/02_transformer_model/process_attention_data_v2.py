...
OUTPUT_DIRS = {
    pairing: os.path.join(BASE_OUTPUT_DIR, f"processed_attention_data_{pairing.lower()}")
    for pairing in ("Leaf", "Root")
}
...
# ===== HELPER FUNCTIONS =====
def load_h5_tensor_data(filepath: str) -> Optional[Dict[str, Any]]:
    """
...
def load_metadata_file(feather_path: str, csv_fallback_path: str, index_col: str) -> Optional[pd.DataFrame]:
    """
...
def process_pairing_attention(pairing_name: str,
    ...
    # --- OPTIONAL: filter augmented samples (recommended) ---
    aug_regex = r"(?:_GP|_WARP|_SCALE|_NOISE|_ADD|_MULT|_MIX)$"   # non-capturing: avoids match-group warning

    # Make mask explicitly a numpy bool array (works whether index.str.contains returns ndarray or Series)
    mask_aug = metadata_df.index.astype(str).str.contains(aug_regex, regex=True, na=False)
    mask_aug = np.asarray(mask_aug, dtype=bool)

    keep = ~mask_aug

    n_aug = int(mask_aug.sum())
    if n_aug > 0:
        logger.warning(f"[{pairing_name}] Filtering augmented samples by suffix regex: {aug_regex}")
        logger.warning(f"[{pairing_name}] Augmented rows detected: {mask_aug.sum()} / {len(mask_aug)}")
        logger.warning(f"[{pairing_name}] Keeping original rows: {keep.sum()} / {len(keep)}")

        # IMPORTANT: subset everything POSITIONALLY (avoid any index alignment surprises)
        metadata_df = metadata_df.iloc[keep].copy()

        attn_s2m_raw = attn_s2m_raw[keep, ...]
        if attn_m2s_raw is not None:
            attn_m2s_raw = attn_m2s_raw[keep, ...]

        logger.warning(f"[{pairing_name}] After filtering: metadata={len(metadata_df)} | "
                       f"S2M={attn_s2m_raw.shape[0]} | M2S={attn_m2s_raw.shape[0] if attn_m2s_raw is not None else 'NA'}")
    else:
        logger.info(f"[{pairing_name}] No augmented samples detected by suffix regex.")
    # --- end filter ---
