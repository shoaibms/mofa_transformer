# -*- coding: utf-8 -*-
"""
Prepare Visualization Data and Generate Validation Figures.

This script generates the 6-panel validation figure for HyperSeq analysis,
visualizing MOFA+ variance, factor loadings, and transformer attention scores.
"""
import pandas as pd
import numpy as np
import os
...
def load_all_data():
    """
    Load and process all required data files for visualization.
    
    Returns:
        dict: A dictionary containing loaded datasets and processed results, or None if loading fails.
    """
    ...
    try:
        ...
        with h5py.File(MOFA_MODEL_PATH, 'r') as hf:
            ...
            # Identify active factors (variance > 2%)
...
def create_validation_figure(data, output_dir, font_sizes):
    """
    ...
    Args:
        ...
        font_sizes (dict): Dictionary of font sizes for plot elements.
    """
    ...
    # --- Panel A: MOFA+ Variance Explained ---
    ax = axes[0, 0]
    ve = data['mofa_variance']
    ...
    ax.set_title("A) MOFA+ Variance Explained", fontsize=font_sizes['panel_title'], fontweight='bold')
    ...
    # --- Panel B & C: Factor Loadings ---
    # Use Factor 3 (integration factor)
    integration_factor_idx = 2  # Factor 3 (0-indexed)
    
    # Panel B: Spectral Loadings
    ax = axes[0, 1]
    ...
    ax.set_title(f"B) Top Spectral Loadings (Factor {integration_factor_idx+1})", 
    ...
    ax.set_title(f"C) Top Gene Loadings (Factor {integration_factor_idx+1})", 
                fontsize=font_sizes['panel_title'], fontweight='bold')
    ax.set_xlabel("Weight", fontsize=font_sizes['axis_label'])
    ax.tick_params(axis='both', labelsize=font_sizes['tick_label'])
    sns.despine(ax=ax)
    
    # --- Panel D: Attention vs. Correlation ---
    ax = axes[1, 0]
    ...
    if len(common_spectral) > 0 and len(common_genes) > 0:
        ...
        if correlations:
            ...
            if len(comparison_df) > 0:
                ...
                ax.set_title("D) Attention vs. Correlation", fontsize=font_sizes['panel_title'], fontweight='bold')
    ...
    if data['permutation_results'] is not None:
        ...
        ax.set_title("E) Permutation Test Results", fontsize=font_sizes['panel_title'], fontweight='bold')
        ...
        ax.set_title("E) Attention Score Significance", fontsize=font_sizes['panel_title'], fontweight='bold')
        ax.legend(fontsize=font_sizes['legend_text'])
        ax.tick_params(axis='both', labelsize=font_sizes['tick_label'])
    ...
    ax.set_title("F) Top Cross-Modal Attention Pairs", fontsize=font_sizes['panel_title'], fontweight='bold')
    ax.set_xlabel("Mean Attention Score", fontsize=font_sizes['axis_label'])
    ax.set_ylabel("")
    ax.tick_params(axis='y', labelsize=font_sizes['tick_label'])
    ...
    output_path = os.path.join(output_dir, "hyperseq_validation.png")
    svg_path = os.path.join(output_dir, "hyperseq_validation.svg")
...
if __name__ == "__main__":
    ...
    if all_data:
        create_validation_figure(all_data, FIGURE_OUTPUT_DIR, FONT_SIZES)
        print("Figure generation complete.")
    else:
        print("Error: Could not load all necessary data.")
