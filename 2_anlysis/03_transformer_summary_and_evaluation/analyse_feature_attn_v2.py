...
def analyze_conditional_attention(transformer_dir, attention_subdir):
    ...
    for tissue in ['Leaf', 'Root']:
        ...
        for day in [1, 2, 3]:
            ...
            # Average across genotypes first, then rank overall top links
            df_t1_d_avg = (
                df_t1_d
                .groupby(['Spectral_Feature', 'Metabolite_Feature'])['Mean_Attention_S2M_Group_AvgHeads']
                .mean()
                .reset_index()
            )

            top_pairs_t1d_overall = get_top_n_pairs(df_t1_d_avg, TOP_N_CONDITIONAL_PAIRS)
