...
# Use the full dataset (203 rows with augmented), then filter to originals only
ATTENTION_DATA_LEAF_PATH = os.path.join(TRANSFORMER_OUTPUT_DIR, "v3_feature_attention", "processed_attention_leaf", "processed_view_level_attention_Leaf.csv")
ATTENTION_DATA_ROOT_PATH = os.path.join(TRANSFORMER_OUTPUT_DIR, "v3_feature_attention", "processed_attention_root", "processed_view_level_attention_Root.csv")
...
# Define the specific comparisons needed based on dummy text
comparisons = [
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Filter to original samples only (exclude augmented)
        # Augmented samples have suffixes like _GP, _WARP, _SCALE, _NOISE, _ADD, _MULT, _MIX
        AUGMENTATION_REGEX = r'_(GP|WARP|SCALE|NOISE|ADD|MULT|MIX)$'
        
        # Get the index column (first column or Row_names)
        index_col = leaf_attn.columns[0] if 'Row_names' not in leaf_attn.columns else 'Row_names'
        
        # Filter Leaf
        n_before_leaf = len(leaf_attn)
        leaf_attn = leaf_attn[~leaf_attn[index_col].astype(str).str.contains(AUGMENTATION_REGEX, regex=True)]
        debug_print(f"  Leaf: Filtered {n_before_leaf} -> {len(leaf_attn)} samples (removed {n_before_leaf - len(leaf_attn)} augmented)")
        
        # Filter Root
        n_before_root = len(root_attn)
        root_attn = root_attn[~root_attn[index_col].astype(str).str.contains(AUGMENTATION_REGEX, regex=True)]
        debug_print(f"  Root: Filtered {n_before_root} -> {len(root_attn)} samples (removed {n_before_root - len(root_attn)} augmented)")
